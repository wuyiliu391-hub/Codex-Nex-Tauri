...
async fn context_window_token_status_with_config(
    ...
    // Include a hysteresis margin to prevent single-token compaction thrashing.
    let hysteresis_margin = auto_compact_scope_limit.map(|l| (l / 50).max(64)).unwrap_or(0);
    let effective_compact_limit = buffered_auto_compact_limit
        .map(|limit| limit.saturating_add(if auto_compact_fallback_buffer_tokens == 0 { hysteresis_margin } else { 0 }));
    let full_context_window_limit_reached =
        full_context_window_limit.is_some_and(|limit| active_context_tokens >= limit);
    let token_limit_reached = effective_compact_limit
