    ...
    let compaction_response_id = loop {
        ...
        match attempt_result {
            ...
            Err(e) if matches!(e.details(), CodexErrorDetails::ContextWindowExceeded) => {
                if turn_input_len > 1 && history.remove_oldest_non_pinned_item() {
                    // Trim non-pinned items starting after the root mission (index 0)
                    // to preserve agent instructions, identity, and goal intact.
                    error!(
                        "Context window exceeded while compacting; removing oldest non-pinned history item. Error: {e}"
                    );
                    retries = 0;
                    continue;
                }
