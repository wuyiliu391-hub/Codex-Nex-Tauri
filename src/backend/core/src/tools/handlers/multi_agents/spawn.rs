...
async fn handle_spawn_agent(
    ...
    let max_depth = turn.config.agent_max_depth.clamp(1, 4);
    if exceeds_thread_spawn_depth_limit(child_depth, max_depth) {
        return Err(FunctionCallError::RespondToModel(
            "Agent depth limit reached (circuit breaker triggered). Solve the task yourself without spawning further sub-agents.".to_string(),
        ));
    }
    session
