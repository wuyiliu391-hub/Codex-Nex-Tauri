...
impl ContextManager {
    ...
    /// Safely removes the oldest non-pinned history item.
    /// Index 0 is pinned (system prompt / initial user mission) to prevent goal corruption and model drift.
    pub(crate) fn remove_oldest_non_pinned_item(&mut self) -> bool {
        if self.items.len() > 1 {
