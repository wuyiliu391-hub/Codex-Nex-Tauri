...
{
    #[cfg(not(test))]
    if std::env::var("CODEX_ENABLE_BACKGROUND_MEMORY_CONSOLIDATION").as_deref() != Ok("1") {
        // Disabled by default: prevents unannounced LLM token consumption and thread pool starvation.
        return Ok(());
    }
